...
async def _get_tradable_dfs(new_sector_list_df:pd.DataFrame, sbi_data_fetcher:SBIDataFetcher) -> Tuple[uc.core.tab.Tab, pd.DataFrame, pd.DataFrame]:
    ...
    return tab, buyable_symbol_codes_df, sellable_symbol_codes_df
...
async def _determine_orders(long_df:pd.DataFrame, short_df:pd.DataFrame, 
                            ...
                            sbi_data_fetcher:SBIDataFetcher) -> Tuple[uc.core.tab.Tab, pd.DataFrame, pd.DataFrame]:
    '''発注銘柄と発注単位の算出'''
    #業種ごとの発注限度額の算出
...
async def _update_trade_history(trade_history_path: str, sector_list_path: str, 
                                sbi_data_fetcher: SBIDataFetcher) -> Tuple[uc.core.tab.Tab, pd.DataFrame]:
    '''
...
async def _update_buying_power_history(buying_power_history_path: str, trade_history: pd.DataFrame, 
    ...
    # 買付余力の取得
    await SBIDataFetcher.get_buying_power()
...
async def select_stocks(sbi_data_fetcher:SBIDataFetcher, order_price_df:pd.DataFrame, new_sector_list_csv:str, y_test_df:pd.DataFrame,
    ...
    tab, buyable_symbol_codes_df, sellable_symbol_codes_df = await _get_tradable_dfs(new_sector_list_df, sbi_data_fetcher)
    ...
    # 注文する銘柄と注文単位数を算出
    tab, long_orders, short_orders = await _determine_orders(long_df, short_df, sectors_to_trade_num, top_slope, sbi_data_fetcher)
...
async def _make_orders(orders_df, order_type_value, sbi_order_maker: SBIOrderMaker) -> list:
    ...
    for symbol_code, unit, L_or_S, cost in zip(orders_df['Code'], orders_df['Unit'], orders_df['LorS'], orders_df['EstimatedCost']):
        ...
        sbi_order_maker.make_order(order_params)
...
async def make_additional_order(sbi_order_maker: SBIOrderMaker) -> list:
    ...
    # TODO 追加注文時のOrderManagerの挙動を適正化する必要がある。
