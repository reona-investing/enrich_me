...
class FinProcessor:
    ...
    def _format_dtypes(self, raw_stock_fin: pd.DataFrame) -> pd.DataFrame:
        '''各カラムに適切なデータ型を設定します。'''
    ...
    def _drop_duplicated_data(self, stock_fin: pd.DataFrame) -> pd.DataFrame:
    ...
    def _calculate_additional_fins(self, stock_fin: pd.DataFrame) -> pd.DataFrame:
        '''追加の財務要素を算出します。'''
        #期末発行済株式数の算出
