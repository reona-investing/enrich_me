...
class OvernightReturn(CalculationMethodBase):
    ...
    def calculate(self, return_timeseries: pd.DataFrame) -> pd.DataFrame:
        ...
        Returns:
            pd.DataFrame: オーバーナイトリターンが計算されたDataFrame (インデックスはreturn_timeseriesと一致)
        '''
        result_df = return_timeseries.copy()
        result_df[self._return_column] = return_timeseries[self._open_column] / return_timeseries[self._close_column].shift(1) - 1
