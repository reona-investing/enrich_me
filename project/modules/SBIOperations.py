...
class SBIOperations:
    ...
    async def _input_sashinari_params(self, order_param_dicts:dict,
        ...
        if order_type == "逆指値":
            ...
            if stop_order_trigger_price is None:
                ...
                choice = await self.tab.select(f'#gsn2 > table > tbody > tr > td:nth-child(2) > select:nth-child(6) > option:nth-child({order_param_dicts["逆指値タイプ"][stop_order_type]})')
    ...
    async def settle_all_margins(tab:uc.core.tab.Tab=None) -> tuple[uc.core.tab.Tab, list]:
        ...
        while i < len(margin_tickers):
            if margin_tickers[i] in ordered_tickers:
                ...
                tab = await _select_pulldown(tab, selector)
                # 取引パスワード
