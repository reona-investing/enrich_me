...
class TestLgbmModel(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # カテゴリ変数も追加（数値型カテゴリとして追加）
        self.X['category1'] = np.random.randint(0, 3, size=n_samples).astype('int')
        self.X['category2'] = np.random.randint(0, 3, size=n_samples).astype('int')
    ...
    def test_custom_eval_metric(self):
        ...
        X_with_date['date'] = np.random.randint(0, 3, size=len(self.X)).astype('int')
