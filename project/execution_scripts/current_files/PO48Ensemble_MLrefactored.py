...
from models.machine_learning_refactored.ml_dataset import MLDatasets, SingleMLDataset
from models.machine_learning_refactored.ensembles import EnsembleMethodFactory
from models.machine_learning_refactored.models import LassoModel, LgbmModel
...
def ensemble_pred_results(ensemble_inputs: list[pd.DataFrame, float]) -> pd.DataFrame:
    ...
    return rank_method.ensemble(ensemble_inputs)


def update_ensembled_model(ENSEMBLED_DATASET_PATH: str | os.PathLike[str], ensembled_pred_df: pd.DataFrame) -> SingleMLDataset:
    single_ml_dataset = SingleMLDataset(ENSEMBLED_DATASET_PATH, 'Ensembled')
    single_ml_dataset.archive_pred_result(ensembled_pred_df)
    single_ml_dataset.save()
    return single_ml_dataset
